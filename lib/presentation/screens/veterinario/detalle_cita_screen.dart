...
class _DetalleCitaScreenState extends State<DetalleCitaScreen> {
  ...
  void initState() {
    super.initState();
    _descripcionCtrl = TextEditingController(text: widget.cita.descripcion ?? '');
  ...
  Color get _estadoColor {
    switch (_estadoActual.toLowerCase()) {
      case 'completada': return const Color(0xFF43B89C);
      case 'cancelada':  return const Color(0xFFE53935);
      default:           return const Color(0xFFE58D57);
    }
  }

  IconData get _estadoIcon {
    switch (_estadoActual.toLowerCase()) {
      case 'completada': return Icons.task_alt_rounded;
                    ...
                    child: Column(children: [
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            _quickStat(Icons.access_time_rounded,
                                widget.cita.hora, 'Hora', _estadoColor),
                            Container(width: 1, height: 40,
                                color: Colors.grey.shade200),
                            _quickStat(_estadoIcon,
                                _estadoActual, 'Estado', _estadoColor),
                          ],
                        ),
                      ),
                      ...
                      _infoCard(
                        ...
                        children: [
                          ...
                          _infoRow(Icons.tag_rounded, 'ID usuario',
                              widget.cita.usuarioId.isNotEmpty
                                  ? widget.cita.usuarioId.substring(0, 8) + '…'
                                  : '—'),
                        ],
                      ),
                      const SizedBox(height: 14),
                      // ── Descripción del veterinario ──
                      _editableCard(
                        title: 'Notas de la consulta',
                        icon: Icons.description_outlined,
                        color: const Color(0xFF7C6FCD),
                        controller: _descripcionCtrl,
                        hint: 'Describe los síntomas, diagnóstico y observaciones…',
                        maxLines: 5,
                        enabled: _estadoActual.toLowerCase() != 'cancelada',
                      ),
                      const SizedBox(height: 14),
                      // ── Receta ──
                      _editableCard(
                      ...
                      const SizedBox(height: 14),
                      // ── Cambiar estado ──
                      if (_estadoActual.toLowerCase() != 'cancelada') ...[
                        _infoCard(
                          ...
                          children: [
                            _estadoSelector('pendiente',
                                Icons.schedule_rounded, const Color(0xFFE58D57)),
  ...
  Widget _editableCard({required String title, required IconData icon,
      required Color color, required TextEditingController controller,
      required String hint, int maxLines = 3, bool enabled = true}) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _estadoSelector(String estado, IconData icon, Color color) {
